...
xAng = cv2.Sobel(gray, cv2.CV_64F, 1, 0)
xAngVis = np.absolute(xAng)
xAngVis = np.clip(xAngVis, 0, 255).astype(np.uint8)

yAng = cv2.Sobel(gray, cv2.CV_64F, 0, 1)
yAngVis = np.absolute(yAng)
yAngVis = np.clip(yAngVis, 0, 255).astype(np.uint8)

diagonal = np.arctan(yAngVis / xAngVis)
angular = np.clip(diagonal, 0, 255).astype(np.uint8)
...
cv2.imshow("45 Degree Angle", angular)
