...
#        
#    for (ex, ey, ex2, ey2) in eye_rects:
#        disp = cv2.rectangle(disp, (ex, ey), (ex2, ey2), (100, 200, 0), 2)
#        angle = np.arctan((ey2-ey)/(ex2-ex)
#        #then rotate the frame it wsa found in
#        
    ...
    if 's' == chr(key& 255): 
        myFace = disp[y:y+h, x:x+w]
        cv2.imwrite("face_test_00" + str(imgName) + ".jpg", myFace)
        imgName += 1
